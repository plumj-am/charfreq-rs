...
pub fn scan_repo(repo_path: &str, args: &Args) -> Result<FinalOutput, ScanError> {
   ...
   let total_chars: u64 =
      char_count.ascii.iter().sum::<u64>() + char_count.unicode.values().sum::<u64>();
                  ...
                  match reader.read_to_end(&mut content) {
                     Ok(_) => {
                        match count_chars(&content) {
                           ...
                           Err(e) => {
                              Some(ScanTaskResult::FileError(format!(
                                 "{}: {}",
                                 path.display(),
                                 e
                              )))
